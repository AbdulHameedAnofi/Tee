    ...
    useEffect(() => {
        if (!isSmallScreen) {
            const timer = setInterval(() => {
                setCurrentSlide((prevSlide) => (prevSlide + 1) % discussionSlides.length)
            }, 4000)
                        ...
                        <div key={index} className="flex flex-col sm:flex-row items-center gap-8">
                            <div className="flex-1">
                                <div className="flex gap-4 mb-4">
                                    ...
                                    <h2 className="text-2xl text-white font-bold">{slide.title}</h2>
                                </div>
                ...
                <div className="relative lg:mx-28">
                        ...
                        >
                            <div className="grid md:grid-cols-2 gap-12 items-center">
                                <div>
                                    ...
                                    <p className="text-white leading-relaxed line-clamp-5">{slide.content}</p>
                                </div>
                    ...
                    <div className="flex justify-center mt-14 cursor-pointer">
                        {discussionSlides.map((_, index) => (
                            <button
                                key={index}
                                className={`w-1/5 rounded-full cursor-pointer ${index === currentSlide ? 'bg-white h-0.9' : 'bg-gray-600 h-0.5'
                                    }`}
